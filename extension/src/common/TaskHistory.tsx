...
const CollapsibleComponent = (props: { title: string; subtitle?: string; text: string }) => (
...
const TaskHistoryItem = ({ index, entry }: TaskHistoryItemProps) => {
    ...
    if ('error' in entry.action || entry.action.parsedAction.name === 'fail') {
        colors.text = 'red.800'
        colors.bg = 'red.100'
    } else if ('parsedAction' in entry.action && entry.action.parsedAction.name === 'finish') {
...
export default function TaskHistory() {
    ...
    return (
        <VStack mt={8}>
            ...
            <Accordion allowMultiple w="full" pb="4" defaultIndex={[0]}>
