...
const optimizeTemplate = (template: PossibleTemplate): OptimizedTemplate => {
    ...
    const valuesToInline = range(template.nodes[0].templateValues.length).filter((i) => {
...
function chooseTemplates(
    ...
    for (const template of Object.values(templates).sort((t) => -t.depth)) {
        ...
        if (
            template.nodes.length - (consumedTemplateCounts[template.hash] ?? 0) < 3 ||
            template.depth < 3
        ) {
            continue
        }
        ...
        const serialized = createTemplateTree(template.nodes[0], chosenTemplates, template)
        template.template = serialized.template
        chosenTemplates[template.hash] = template
        serialized.consumedTemplates.forEach((t) => {
            consumedTemplateCounts[t] = (consumedTemplateCounts[t] ?? 0) + template.nodes.length
        })
    }
...
function getPlaceholder(template: OptimizedTemplate, valueIndex: number): string {
    // valueIndex plus one minus the number of values below it that are inlined
    const placeholderIndex =
        valueIndex + 1 - Array.from(template.valuesToInline).filter((i) => i < valueIndex).length
...
function createTemplateTree(
    ...
    return {
        template: `<${node.tagName.toLowerCase()}${attrs}${
            isSelfClosing ? '/>' : `>${children.join('')}</${node.tagName.toLowerCase()}>`
        }`,
...
function serializeTree(node: JsonNode, templates: Record<string, OptimizedTemplate>): string {
    if (node.type === 'TEXT') {
        return node.content
    }
    ...
    const children = node.children.map((child) => serializeTree(child, templates)).join('')
